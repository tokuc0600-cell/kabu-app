...
### ショート対応（株・FX両対応・2026-06-25〜26）

- `backtest/strategy.py` の `PositionState` に `SHORT` を追加。`should_enter()`/`check_exit_by_pct()`/
  `check_exit_by_pct_intrabar()`/`check_exit_by_pips()`/`check_exit_by_pips_intrabar()`/`should_exit()`/
  `step_position()`/`detect_rci_signal_series()` に`direction`キーワード引数（`"long"`既定 / `"short"`）を
  追加し、エントリー/エグジットのクロス判定・損切利確の不等号・PnL計算をショートでは反転させる。
  `direction`を渡さない既存呼び出し元（`scripts/check_exit_signals.py`等）は今までと完全に同一の挙動。
- 株は2026-06-25に先行対応（%ベース）、FXは2026-06-26にpipsベースの`check_exit_by_pips`/
  `check_exit_by_pips_intrabar`にも同様の`direction`対応を追加し、`backtest/engine.py build_trades()`の
  「FXは常にlong扱いに固定する」分岐を解除して両対応した。
- **通知スクリプトは対象外**：`scripts/check_exit_signals.py`は全銘柄一律%・ロング固定のまま変更しない
  （株の通知用途に限定したシンプル化方針のため）。
- Sheets「ウォッチリスト」シートに**M列「売買方向」**、「FXウォッチリスト」シートに**N列「売買方向」**
  （値："ロング"/"ショート"、空欄は"ロング"扱いで後方互換）を追加。`sync_kabu.py`/`sync_fx.py`それぞれの
  `_resolve_direction()`がこの列を読む。
...
- ワークブック名：「kabu」
- シート①：「FXウォッチリスト」（通貨ペア名, Yahooティッカー, 現在値, 20EMA, 200EMA, 20EMA乖離率, トレンド状態, シグナル, 最終更新日時, **損切りpips, 利確pips, 建値, ポジション状態, 売買方向（N列）**）
- シート②：「ウォッチリスト」（銘柄コード, 銘柄名, 現在値, 25日移動平均, 25日乖離率, シグナル, **損切り%, 利確%, 建値, ポジション状態, 最終更新日時, 売買方向（M列）**）
...
| ポジション状態 | "ノーポジ" / "ロング中" / "ショート中" | エントリー・エグジット時に自動更新 |
| 売買方向（株M列・FX N列） | "ロング" / "ショート"（空欄は"ロング"扱い） | 手入力、またはチャート分析タブのポジション操作で記録 |
...
- [x] Phase 7：FXダッシュボードへのショート対応・新規テクニカル指標・選択式チャート指標の展開（2026-06-26）
  - `strategy.py`のpipsベース関数（`check_exit_by_pips`/`check_exit_by_pips_intrabar`）に`direction`対応を追加し、
    `backtest/engine.py`のFX強制ロング化分岐を解除。株で先行実装した仕組みをFXにも展開
  - FXウォッチリストにも売買方向（N列）・テクニカルサマリー・選択式チャート指標・バックテスト方向選択を追加（株と同構成）
