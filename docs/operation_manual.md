...
### 2.12 FXバックテストの結果表示を純利益ベースに変更（2026-06-24）

- ユーザーから「プロフィットファクター（PF）1.22は利益が出ているように見えるが、実際の資産の増減が分かりにくい」との指摘があり、`backtest/engine.py`の`summarize()`に純利益（`net_profit` = 勝ちトレード合計 − 負けトレード合計、pips/%単位）を追加した。
- `streamlit_dashboard/fx/app_fx.py`のタブ3「バックテスト」結果表示は、4枚のメトリクスのうち「プロフィットファクター」を「純利益」に置き換えた（取引回数・勝率・純利益・最大ドローダウンの4つに変更）。PFは比率であり連敗による資金の谷（最大ドローダウン）が見えないため、実際の損益感覚に近い純利益を優先表示する方針とした。
